...
pub fn run() {
    let sidecar_state: SharedSidecar = Arc::new(Mutex::new(SidecarManager::new()));
            ...
            move |app| {
                ...
                for dir in &dirs_to_create {
                    if let Err(e) = std::fs::create_dir_all(dir) {
                        log::error!("Failed to create dir {}: {e}", dir.display());
                    }
                }
                ...
                let db_state: DbConn = Arc::new(Mutex::new(conn));
                app.manage(db_state);
